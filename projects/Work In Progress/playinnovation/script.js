docReady(function() {
    ...
    const explanationImagesHref = document.querySelectorAll('.template-widget.image.column-right a');
    ...
    const totalDifference = parseInt(customVars[0].innerHTML);
    const overallDifference = parseInt(customVars[1].innerHTML);
    const exploreDifference = parseInt(customVars[2].innerHTML);
    const developDifference = parseInt(customVars[3].innerHTML);
    const proveDifference = parseInt(customVars[4].innerHTML);
    const monitorDifference = parseInt(customVars[5].innerHTML);
    const clpdDifference = parseInt(customVars[6].innerHTML);
    const plrDifference = parseInt(customVars[7].innerHTML);
    const flpDifference = parseInt(customVars[8].innerHTML);
    const iiDifference = parseInt(customVars[9].innerHTML);
    // Set background image for summary

    if (totalDifference >= 3) {
        pageBreaks[2].children[0].children[0].children[0].src = "https://surveyanyplace.s3.amazonaws.com/app/editor/reports/pdf/custom/ahead_game.png";
        explanationImagesHref[0].href = ('https://www.playinnovation.com.au/innovation-calculator-above-average-result');
    } else if (totalDifference >= -3 && totalDifference < 3) {
        pageBreaks[2].children[0].children[0].children[0].src = "https://surveyanyplace.s3.amazonaws.com/app/editor/reports/pdf/custom/on_track.png";
        explanationImagesHref[0].href = ('https://www.playinnovation.com.au/innovation-calculator-average-result');
    } else {
        pageBreaks[2].children[0].children[0].children[0].src = "https://surveyanyplace.s3.amazonaws.com/app/editor/reports/pdf/custom/behind_curve.png";
        explanationImagesHref[0].href = ('https://www.playinnovation.com.au/innovation-calculator-below-average-result');
    }
